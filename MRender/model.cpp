...
void Model::createMap(const char* filename)
{
	...
	if (_access(texfile.data(), 0) != -1)
	{
		...
		loadTexture(filename, "_normal.tga", normalmap);
	}
	...
	if (_access(texfile.data(), 0) != -1)
	{
		...
		loadTexture(filename, "_roughness.tga", roughnessmap);
	}
	...
	if (_access(texfile.data(), 0) != -1)
	{
		...
		loadTexture(filename, "_metalness.tga", metalnessmap);
	}
	...
	if (_access(texfile.data(), 0) != -1)
	{
		...
		loadTexture(filename, "_emission.tga", emision_map);
	}
	...
	if (_access(texfile.data(), 0) != -1)
	{
		...
		loadTexture(filename, "_occlusion.tga", metalnessmap);
	}
}
...
{
	return verts[faces[iface][nthvert * 3]]; //obj文件格式，f开头的前三个是面的顶点索引
}
