...
static LRESULT CALLBACK msg_callback(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		break;
	case WM_LBUTTONDOWN:
		window->mouse_info.orbit_pos = getMousePos();
		window->buttons[0] = 1; 
		break;
	case WM_LBUTTONUP:
		window->buttons[0] = 0;
		break;
	case WM_RBUTTONDOWN:
		window->mouse_info.fv_pos = getMousePos();
		window->buttons[1] = 1;
		break;
	case WM_RBUTTONUP:
		window->buttons[1] = 0;
...
static void registerWindowClass()
{
	...
	wc.lpszClassName = "MRender";					//该窗口类的名字
...
int initWindow(int width, int height, const char* title) {
	...
	RECT rect = { 0, 0, width, height }; //创建一个矩形范围
	int wx, wy, sx, sy;
	LPVOID ptr;
	HDC hdc;
	BITMAPINFOHEADER bi;
	...
	//创建窗口
	window->h_window = CreateWindow("MRender", title,
